...
optimize_image()
{
	...
	if [ "${1##*.}" = "png" ]; then
		optipng -o1 -quiet $1 -out $2
		pngcrush -q -rem alla -reduce $1 "$2.tmp" && mv "$2.tmp" "$1"
# >/dev/null
	fi
	if [ "${1##*.}" = "jpg" -o "${1##*.}" = "jpeg" ]; then
		jpegtran -copy none -progressive -outfile $1 $1 
	fi
...
#	break
	...
	for CURRENT_IMAGE in $IMAGES; do
		...
		if [ "$QUIET" == "0" ]; then
		    ...
		    imglvlup=$( printf "/home/admin/web/ovorobev011.fvds.ru/public_html/2" )
		    imgdir=$( echo $CURRENT_IMAGE | grep -o -e '\/img_test\/.*\/' )
		    # echo $imglvlup
		    # echo $imgdir
		    # OUTPUT=$( $imglvlup$imgdir$filename) #и снова обратите внимание на скобки и пробелы
		    #echo $OUTPUT

		    #OUTPUT=$( echo $CURRENT_IMAGE | grep -o -e '.*img_test\/.*\/' )
		fi

		#optimize_image $CURRENT_IMAGE $OUTPUT/$filename
		resize_image $CURRENT_IMAGE $CURRENT_IMAGE
		optimize_image $CURRENT_IMAGE $CURRENT_IMAGE
